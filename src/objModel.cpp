...
//船的反射系数
glm::vec4 diffuse = glm::vec4(0.1f, 0.1f, 0.12f, 0.1f);
glm::vec4 ambient = glm::vec4(0.1f, 0.1f, 0.1f, 0.1f);
glm::vec4 specular = glm::vec4(0.1f, 0.1f, 0.1f, 0.1f);
float shine = 50.0f;
...
objModel::objModel(const char* objFileName, const char* textureFileName,float factor) {
    ...
    for (int i = 0; i < vertexIndices.size(); i++) {
        ...
        final_verticesTextures.push_back(final_normals[3 * normalIndices[i]]);
        final_verticesTextures.push_back(final_normals[3 * normalIndices[i] + 1]);
        final_verticesTextures.push_back(final_normals[3 * normalIndices[i] + 2]);
    }
...
void objModel::objModelOut() {
    FILE* file = fopen("../OutPutModel.obj", "w");
    if (file == NULL) {
        cout << "Failed to open the obj file!" << endl;
        exit(0);
    }
    int i = 0;
    for (auto key : final_vertices) {
        float temp = key / factor;
        if(i%3==0)fprintf(file, "v");
        if(i%3==2)fprintf(file, " %f\n", temp);
        else fprintf(file, " %f", temp);
        i++;
    }
    i = 0;
    for (auto key : final_textures) {
        float temp = 1.0-key;
        if (i % 2 == 0)fprintf(file, "vt");
        if (i % 2 == 1)fprintf(file, " %f\n", temp);
        else fprintf(file, " %f", key);
        i++;
    }
    i = 0;
    for (auto key : final_normals) {
        float temp = key / factor;
        if (i % 3 == 0)fprintf(file, "vn");
        if (i % 3 == 2)fprintf(file, " %f\n", temp);
        else fprintf(file, " %f", temp);
        i++;
    }
    ...
    for (i = 0; i < vertexIndices.size();i++) {
        if (i % 3 == 0)fprintf(file, "f");
        if (i % 3 == 2)fprintf(file, " %d/%d/%d\n", vertexIndices[i],textureIndices[i],normalIndices[i]);
        else fprintf(file, " %d/%d/%d", vertexIndices[i], textureIndices[i], normalIndices[i]);
    }
    fclose(file);
}
void objModel::draw(Shader& shader, glm::mat4 model, glm::mat4 view, glm::mat4 projection,glm::vec3 CameraPos, glm::vec3 LightPos,glm::vec3 LightInfo, float LightIntensity) {

    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, texture);

    shader.use();
    shader.setVec4("aDiffuse", diffuse);
    shader.setVec4("aAmbient", ambient);
    shader.setVec4("aSpecular", specular);
    shader.setFloat("aShine", shine);
    LightIntensity = (LightIntensity < 0.15) ? 0.15: LightIntensity;
    shader.setFloat("LightIntensity",LightIntensity);
    //shine = LightIntensity;
    shader.setVec3("LightPos",LightPos);
    shader.setVec3("LightDiffuse",LightInfo);
    shader.setVec3("LightAmbient",LightInfo);
