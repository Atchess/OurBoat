...
GLfloat Ocean::PhillipsSpectrum(int n, int m) {
    vec2 k(acos(-1) * (2 * n - N) / length,acos(-1) * (2 * m - N) / length);
...
GLboolean Ocean::Init()
{
	...
	//oceanShader = new Shader("../shader/terrain.vs", "../shader/terrain.frag");
	oceanShader = new Shader("../shader/water.vs", "../shader/water.fs");
	...
	//声明更新海面网格的计算着色器
	updateHtShader = new Shader("../shader/OceanUpdate.comp");
	//声明进行FFT的计算着色器
	fftShader = new Shader("../shader/OceanFFT.comp");
	//声明更新法线向量的计算着色器
	updateNormalShader = new Shader("../shader/OceanUpdateNormal.comp");
...
////////////////////////////////////////////////////////////////////////////////////////
	...
	oceanShader->setFloat("lightIntensity", lightIntensity);
...
glm::vec3 Ocean::getLightInfo() {
	return deepWaterColorSunny;
}